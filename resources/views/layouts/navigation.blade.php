<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
    ...
    <div class="flex justify-between h-16">
      <div class="flex">
        ...
        <div class="hidden space-x-4 sm:-my-px sm:ms-10 sm:flex">
          {{-- Accueil (=/books) --}}
          <x-nav-link :href="route('home')" :active="request()->routeIs('books.index')">
            {{ __('Accueil') }}
          </x-nav-link>

          {{-- Livres (liste principale) --}}
          <x-nav-link :href="route('books.index')" :active="request()->routeIs('books.index')">
            {{ __('Livres') }}
          </x-nav-link>

          {{-- Nouveautés (lien indépendant) --}}
  ...
  <div :class="{ 'block': open, 'hidden': !open }" class="hidden sm:hidden">
    <div class="pt-2 pb-3 space-y-1">
      <x-responsive-nav-link :href="route('home')" :active="request()->routeIs('books.index')">Accueil</x-responsive-nav-link>
      <x-responsive-nav-link :href="route('books.index')">Livres</x-responsive-nav-link>
      <x-responsive-nav-link :href="route('books.news')" :active="request()->routeIs('books.news')">Nouveautés</x-responsive-nav-link>
      <x-responsive-nav-link :href="route('messages.create')">Contacter Nous</x-responsive-nav-link>
      <x-responsive-nav-link :href="route('cart.index')">Mon Panier</x-responsive-nav-link>
      @auth
        <x-responsive-nav-link :href="route('orders.index')">Mes achats</x-responsive-nav-link>
      @endauth
      @if(auth()->check() && auth()->user()->role === 'admin')
        <x-responsive-nav-link :href="route('admin.messages.index')">Messages</x-responsive-nav-link>
      @endif
    </div>
